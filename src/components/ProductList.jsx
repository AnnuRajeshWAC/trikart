    ...
    <>
      <div className="d-flex justify-content-center p-5 gap-5">
        ...
        <select
          name=""
          id=""
          value={searchParam.get("index") || "English"}
      ...
      <div className="d-flex p-3">
        ...
        <div className="p-3">
            ...
            {allValues?.map((value) =>
              value[0] !== "query" && value[0] !== "page" ? (
                <div className="d-flex" key={value[1]}>
                  <CloseButton
                    onClick={() => removeFilter(value[0], value[1])}
                  />
                  <p>{value[1]}</p>
                </div>
              ) : null
          ...
          <Filters
            products={products?.data?.filter_list}
...
const useProductData = () => {
  ...
  const indexKey = languageConfigs.find(
    (lan) => lan.label === searchParam.get("index")
  )?.options || {
    "Client-id": "7645129791",
    "Secret-key": "Qfj1UUkFItWfVFwWpJ65g0VfhjdVGN",
    "Content-Type": "application/json",
  ...
  const {
    ...
    isLoading,
    error,
  } = useSWR("search", fetcher, { keepPreviousData: true });
